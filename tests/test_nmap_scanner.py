...
class TestNmapScanner(unittest.TestCase):
    def test_scan_returns_two_hosts(self):
        self.assertEqual(len(_make_scanner().parse()), 2)
    ...
    def test_host_dict_keys(self):
        hosts = _make_scanner().parse()
        required = {"ip","hostname","state","os","os_accuracy","open_ports","risky_port_count","scanned_at"}
        for h in hosts:
    ...
    def test_host_with_no_ports(self):
        data = {"192.168.1.50": {"state":"up","hostnames":[{"name":"silent"}],"osmatch":[],"protos":{}}}
